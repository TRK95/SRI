...
// Type definitions for Conductor data types
...
export type TypedValue<T extends DataType> = {
  type: T;
  value: any;
};

export type PairIdentifier = string;
export type ArrayIdentifier<T extends DataType = DataType> = string;
export type ClosureIdentifier<T extends DataType = DataType> = string;
...
export type ExternCallable<T extends IFunctionSignature> = (...args: TypedValue<DataType>[]) => Promise<TypedValue<T['returnType']>>;
...
export type List = TypedValue<DataType.LIST>; 